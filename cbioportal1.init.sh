...
set -x
stage=$1
if [ -z $1 ]; then echo "input a stage: run_mysql, build_cbio, or run_cbio"; exit; fi
...
if [ $stage == 'run_mysql' ]; then
  ...
  echo "docker exec -it ${db_host} /bin/bash -c \"mysql -h${db_host} -u${db_user} -p${db_password} ${portal_db_name}\""
fi
...
# docker exec -it cbioPortal1 /bin/bash -c ""
